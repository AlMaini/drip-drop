...
async def try_on_clothes(
    ...
    try:
        ...
        if len(processed_images) < 2:
            raise HTTPException(status_code=400, detail="At least 2 images required (person + clothing)")
        ...
        for i in range(0, len(clothing_images), batch_size):
            ...
            try:
                ...
                if batch_items == 1:
                    prompt = f"Make the person in the first image wear the {current_descriptions[0]} shown in the second image. Create a realistic visualization of how the clothing item would look when worn by the person, maintaining proper fit, proportions, and styling. Do not change the colors of the clothes. Maintain the pose of the person."
                else:
    ...
    try:
        ...
        for item in items_list:
            try:
                ...
                prompt = f"Take the {item} in this photo and make a full view image of just that item with a white background as a professionally shot image for a clothing item on an online store. Focus only on the {item} and exclude all other clothing items or objects. crop/reframe it to a 1:1 square format. Make it a perfect square image."
